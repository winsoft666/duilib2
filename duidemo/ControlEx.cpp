...
void CChartViewUI::DoPaintPie(HDC hDC, const RECT& rcPaint)
{
	...
	int drawMaxSize = (drawWidth < drawHeight ? drawWidth : drawHeight);
	...
	for (int i = 0; i < numOfItem; i++)
	{
		...
		if (m_bShowText)
		{
			...
			if (::IntersectRect(&m_rcPaint, &rcText, &m_rcItem))
			{
				...
				if( m_bShowHtml )
				{
					CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, m_items[i].name.GetData(), clrColor, NULL, NULL, nLinks, m_iFont, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
				}
				else
				{
					CRenderEngine::DrawText(hDC, m_pManager, rcText, m_items[i].name.GetData(), clrColor, m_iFont, DT_LEFT | DT_VCENTER | DT_SINGLELINE);
				}
			}
...
void CChartViewUI::DoPaintHistogram(HDC hDC, const RECT& rcPaint)
{
	...
	for (i = 0; i < numOfItem; i++)
	{
		...
		if (m_bShowText)
		{
			...
			if( m_bShowHtml )
			{
				CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, m_items[i].name.GetData(), clrColor, NULL, NULL, nLinks, m_iFont, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
			}
			else
			{
				CRenderEngine::DrawText(hDC, m_pManager, rcText, m_items[i].name.GetData(), clrColor, m_iFont, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
			}
		}
