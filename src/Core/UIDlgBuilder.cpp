            ...
            if (_tcsicmp(pstrClass, _T("Window")) == 0) {
                if (pManager->GetPaintWindow()) {
                    ...
                    for (int i = 0; i < nAttributes; i++) {
                        ...
                        if (_tcsicmp(pstrName, _T("dpi")) == 0) {
                            int dpi = 0;

                            if (_tcsicmp(pstrValue, _T("system")) == 0)
                                dpi = pManager->GetDPIObj()->GetMainMonitorDPI();
                            else
                                dpi = _ttoi(pstrValue);

                            if (dpi > 0)
                                pManager->GetDPIObj()->SetScale(dpi);
            ...
            for( CMarkupNode node = root.GetChild() ; node.IsValid(); node = node.GetSibling() ) {
                ...
                if( _tcsicmp(pstrClass, _T("Image")) == 0 ) {
                    ...
                    if( id >= 0 ) {
                        ...
                        if (defaultfont) {
                            // Jeffery: 修复在未指定字体，使用默认字体时，文字被进行2次DPI Scale的BUG，SetDefaultFont内部会进行Scale
                            //
                ...
                if( pManager->GetPaintWindow() ) {
                    ...
                    for( int i = 0; i < nAttributes; i++ ) {
                        ...
                        if( _tcsicmp(pstrName, _T("size")) == 0 ) {
                            ...
                            if (_tcsicmp(pstrValue, TEXT("max")) == 0) {
                                RECT screen_rect = { 0 };
                                BOOL ret = SystemParametersInfo(SPI_GETWORKAREA, 0, &screen_rect, 0);
                                ASSERT(ret);
                                cx = screen_rect.right - screen_rect.left;
                                cy = screen_rect.bottom - screen_rect.top;
                                bDpiScale = false;
                            }
                            else {
                                LPTSTR pstr = NULL;
                            ...
                            pManager->SetInitSize(cx, cy, bDpiScale);
