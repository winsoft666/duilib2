...
namespace DuiLib {
    ...
    void CShadowUI::Create(CPaintManagerUI *pPaintManager) {
        ...
        // Already initialized
        if (CPaintManagerUI::GetInstance() == INVALID_HANDLE_VALUE || pPaintManager == NULL) {
            return;
        }
        ...
        switch(uMsg) {
            ...
            case WM_MOVE:
                if(pThis->m_Status & SS_VISABLE) {
                    RECT WndRectTmp;
                    GetWindowRect(hwnd, &WndRectTmp);

                    if (pThis->m_bIsImageMode) {
                        SetWindowPos(pThis->m_hWnd, hwnd, WndRectTmp.left - pThis->m_nSize, WndRectTmp.top - pThis->m_nSize, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE);
                    } else {
                        SetWindowPos(pThis->m_hWnd, hwnd, WndRectTmp.left + pThis->m_nxOffset - pThis->m_nSize, WndRectTmp.top + pThis->m_nyOffset - pThis->m_nSize, 0, 0, SWP_NOSIZE | SWP_NOACTIVATE);
                    }
                }
