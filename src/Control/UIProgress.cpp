...
namespace DuiLib {
    ...
    void CProgressUI::PaintForeImage(HDC hDC) {
        ...
        if( !m_sForeImage.IsEmpty() ) {
            ...
            int sw = MulDiv(rc.right - rc.left, 100, m_pManager->GetDPIObj()->GetScale());
            int sh = MulDiv(rc.bottom - rc.top, 100, m_pManager->GetDPIObj()->GetScale());
            rc.left = MulDiv(rc.left, 100, m_pManager->GetDPIObj()->GetScale());
            rc.top = MulDiv(rc.top, 100, m_pManager->GetDPIObj()->GetScale());
            rc.right = rc.left + sw;
            rc.bottom = rc.top + sh;
