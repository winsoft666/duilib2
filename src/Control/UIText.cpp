...
namespace DuiLib {
    ...
    SIZE CTextUI::EstimateSize(SIZE szAvailable) {
        ...
        RECT rcText = { 0, 0, m_bAutoCalcWidth ? 9999 : m_pManager->GetDPIObj()->Scale(m_cxyFixed.cx), 9999 };
    ...
    void CTextUI::PaintText(HDC hDC) {
        ...
        if( IsEnabled() ) {
            if( m_bShowHtml )
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText, m_dwTextColor, m_rcLinks, m_sLinks, m_nLinks, m_iFont, m_uTextStyle);
            else
                CRenderEngine::DrawText(hDC, m_pManager, rc, sText, m_dwTextColor, m_iFont, m_uTextStyle);
        } else {
            if( m_bShowHtml )
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText, m_dwDisabledTextColor, m_rcLinks, m_sLinks, m_nLinks, m_iFont, m_uTextStyle);
            else
