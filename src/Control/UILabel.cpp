...
namespace DuiLib {
    ...
    CLabelUI::CLabelUI() :
        m_uTextStyle(DT_VCENTER | DT_SINGLELINE), m_dwTextColor(0),
    ...
    SIZE CLabelUI::EstimateSize(SIZE szAvailable) {
        if (m_cxyFixed.cx > 0 && m_cxyFixed.cy > 0) {
            // Jeffery: 修复了DPI不为100时, CLabelUI(含派生于它的CButtonUI控件)的size不正确的问题
            //
        ...
        if (m_bNeedEstimateSize) {
            ...
            if ((m_uTextStyle & DT_SINGLELINE) != 0) {
                if (m_cxyFixedLast.cy == 0) {
                    ...
                    m_cxyFixedLast.cy += m_pManager->GetDPIObj()->Scale(m_rcTextPadding.top + m_rcTextPadding.bottom);
                }
                else {
                    m_cxyFixedLast.cy = m_pManager->GetDPIObj()->Scale(m_cxyFixedLast.cy);
                    if (m_cxyFixedLast.cy > szAvailable.cy)
                ...
                if (m_cxyFixedLast.cx == 0) {
                    if(m_bAutoCalcWidth) {
                        ...
                        m_cxyFixedLast.cx = rcText.right - rcText.left + m_pManager->GetDPIObj()->Scale(m_rcTextPadding.left + m_rcTextPadding.right);
                    } else {
                        m_cxyFixedLast.cx = szAvailable.cx;
                    }
                }
                else {
                    m_cxyFixedLast.cx = m_pManager->GetDPIObj()->Scale(m_cxyFixedLast.cx);
                    if (m_cxyFixedLast.cx > szAvailable.cx)
                        m_cxyFixedLast.cx = szAvailable.cx;
                }
            } else {
                if( m_cxyFixedLast.cx == 0 ) {
                ...
                if(m_cxyFixedLast.cy == 0) {
                    if(m_bAutoCalcHeight) {
                        ...
                        m_cxyFixedLast.cy = rcText.bottom - rcText.top + m_pManager->GetDPIObj()->Scale(m_rcTextPadding.top + m_rcTextPadding.bottom);
                    } else {
                        m_cxyFixedLast.cy = szAvailable.cy;
                    }
                }
