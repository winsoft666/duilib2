...
#define WM_MENUCLICK WM_USER + 121  //用来接收按钮单击的消息
    ...
    class UILIB_API MenuObserverImpl : public MenuMenuObserverImplBase {
      ...
      public:
        MenuObserverImpl():
            m_pNotifyRecvPaintManager(NULL) {
        ...
        virtual void SetNotifyRecvPaintManager(CPaintManagerUI *pNotifyRecvPM) {
            if (pNotifyRecvPM != NULL)
                m_pNotifyRecvPaintManager = pNotifyRecvPM;
        }

        virtual CPaintManagerUI *GetNotifyRecvPaintManager() const {
            return m_pNotifyRecvPaintManager;
        }

        virtual void SetMenuItemInfo(CStdStringPtrMap *pInfo) {
            if (pInfo != NULL)
                m_pMenuItemInfo = pInfo;
        }

        virtual CStdStringPtrMap *GetMenuItemInfo() const {
            return m_pMenuItemInfo;
        }

      protected:
        typedef std::vector<MenuMenuReceiverImplBase *> ReceiversVector;
        ReceiversVector *pReceivers_;
        CPaintManagerUI *m_pNotifyRecvPaintManager;
        CStdStringPtrMap *m_pMenuItemInfo;
    };
    ...
    class UILIB_API CMenuElementUI : public CListContainerElementUI {
      ...
      public:
        ...
        void SetMenuEnabled(bool bEnabled = true);
        ...
        MenuItemInfo *SetItemInfo(LPCTSTR pstrName, bool *bChecked = NULL, bool *bEnabled = NULL);
      protected:
        CMenuWnd	*m_pWindow;
