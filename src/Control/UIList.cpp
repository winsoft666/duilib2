...
namespace DuiLib {
    ...
    CListUI::CListUI() : m_pCallback(NULL), m_bItemScrollSelect(false), m_iCurSel(-1), m_iExpandedItem(-1), m_bMultiSel(false) {
        ...
        m_ListInfo.dwRowLineHeight = 1;
        m_ListInfo.dwColumnLineWidth = 1;
    ...
    bool CListUI::RemoveAt(int iIndex, bool bResetSelIndex /* = true*/) {
        if (!m_pList->RemoveAt(iIndex))
            return false;
    ...
    bool CListUI::GetItemScrollSelect() {
        return m_bItemScrollSelect;
    }

    void CListUI::SetItemScrollSelect(bool bScrollSelect) {
        m_bItemScrollSelect = bScrollSelect;
    }
    ...
    bool CListUI::UnSelectItem(int iIndex, bool bOthers) {
        ...
        if(bOthers) {
            for (int i = m_aSelItems.GetSize() - 1; i >= 0; --i) {
                ...
                IListItemUI *pSelListItem = static_cast<IListItemUI *>(pControl->GetInterface(DUI_CTR_LISTITEM));
    ...
    RECT CListUI::GetItemTextPadding() const {
        ...
        m_pManager->GetDPIObj()->Scale(&rect);
        return rect;
    }
    ...
    void CListUI::SetItemRowLineHeight(DWORD dwHeight) {
        m_ListInfo.dwRowLineHeight = dwHeight;
        Invalidate();
    }

    void CListUI::SetItemColumnLineWidth(DWORD dwWidth) {
        m_ListInfo.dwColumnLineWidth = dwWidth;
        Invalidate();
    }
    ...
    bool CListUI::ExpandItem(int iIndex, bool bExpand /*= true*/) {
        if( m_iExpandedItem >= 0 && !m_ListInfo.bMultiExpandable) {
            ...
            if( pControl != NULL ) {
                IListItemUI *pItem = static_cast<IListItemUI *>(pControl->GetInterface(DUI_CTR_LISTITEM));
    ...
    void CListUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
        ...
        else if( _tcsicmp(pstrName, _T("itemscrollselect")) == 0 )
            SetItemScrollSelect(_tcsicmp(pstrValue, _T("true")) == 0);
        ...
        else if (_tcsicmp(pstrName, _T("itemrowlineheight")) == 0) {
            SetItemRowLineHeight(_ttoi(pstrValue));
        } else if (_tcsicmp(pstrName, _T("itemcolumnlinewidth")) == 0) {
            SetItemColumnLineWidth(_ttoi(pstrValue));
    ...
    BOOL CListBodyUI::SortItems(PULVCompareFunc pfnCompare, UINT_PTR dwData) {
        ...
        for (int i = 0; i < m_items.GetSize(); ++i) {
            pItem = (IListItemUI *)(static_cast<CControlUI *>(m_items[i])->GetInterface(DUI_CTR_LISTITEM));
    ...
    void CListBodyUI::SetScrollPos(SIZE szPos, bool bMsg) {
        ...
        if( m_pOwner ) {
            ...
            for( int i = 0; i < pInfo->nColumns; i++ ) {
                ...
                RECT rcPos2 = pControl->GetPos();
                rcPos2.left -= cx;
                rcPos2.right -= cx;
                pControl->SetPos(rcPos2);
    ...
    SIZE CListHeaderUI::EstimateSize(SIZE szAvailable) {
        ...
        if( cXY.cy == 0) {
            ...
            // Jeffery: 当用户指定了表头高度时，将高度进行DPI缩放
            //
            if (m_pManager) {
    ...
    void CListHeaderUI::SetPos(RECT rc, bool bNeedInvalidate) {
        ...
        for( int it2 = 0; it2 < m_items.GetSize(); it2++ ) {
            ...
            if (m_bIsPercentageHeader) {
                CListHeaderItemUI *pHeaderItem = static_cast<CListHeaderItemUI *>(pControl);
                sz.cx = int(nHeaderWidth * (float)pHeaderItem->GetPercent() / 100);
            } else {
    ...
    void CListHeaderUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
        if( _tcsicmp(pstrName, _T("percentageheader")) == 0 )
            SetPercentageHeader(_tcsicmp(pstrValue, _T("true")) == 0);
        else
            CHorizontalLayoutUI::SetAttribute(pstrName, pstrValue);
    }

    void CListHeaderUI::SetPercentageHeader(bool bIsPercentage) {
        m_bIsPercentageHeader = bIsPercentage;
    }

    bool CListHeaderUI::IsPercentageHeader() const {
        return m_bIsPercentageHeader;
    }
    ...
    bool CListHeaderItemUI::GetSepWidthAdaptDpiScale() {
        return m_bSepWidthAdaptDpiScale;
    }

    void CListHeaderItemUI::SetSepWidthAdaptDpiScale(bool adapt) {
        m_bSepWidthAdaptDpiScale = adapt;
    }
    ...
    void CListHeaderItemUI::SetPercent(int nPercent) {
        m_nScale = nPercent;
    }

    int CListHeaderItemUI::GetPercent() const {
        return m_nScale;
    }
    ...
    void CListHeaderItemUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue) {
        ...
        else if (_tcsicmp(pstrName, _T("sepwidthadaptdpiscale")) == 0) {
            SetSepWidthAdaptDpiScale(_tcsicmp(pstrValue, _T("true")) == 0);
        ...
        else if( _tcsicmp(pstrName, _T("percent")) == 0 ) {
            LPTSTR pstr = NULL;
            SetPercent(_tcstol(pstrValue, &pstr, 10));
        } else {
    ...
    void CListHeaderItemUI::DoEvent(TEventUI &event) {
        ...
        if( event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_DBLCLICK ) {
            ...
            RECT rcSeparator = GetSepThumbRect();
        ...
        if( event.Type == UIEVENT_MOUSEMOVE ) {
            if( (m_uButtonState & UISTATE_CAPTURED) != 0 ) {
                ...
                if( rc.right - rc.left > GetMinWidth() ) {
                    // Jeffery: 修复拖动鼠标改变表头宽度时，表头宽度成倍数级增长的BUG。因为m_rcItem每次都是由EstimateSize修改，而EstimateSize中会根据m_cxyFixed进行DPI缩放。
                    //          也就是说，m_rcItem中存放的是DPI缩放之后的值，而m_cxyFixed存放的是未经DPI缩放的值。
                    //
        ...
        if( event.Type == UIEVENT_SETCURSOR ) {
            ...
            if( IsEnabled() && m_bDragable && ::PtInRect(&rcSeparator, event.ptMouse) ) {
                ::SetCursor(::LoadCursor(NULL, MAKEINTRESOURCE((DWORD)IDC_SIZEWE)));
                return;
            }
        }
    ...
    SIZE CListHeaderItemUI::EstimateSize(SIZE szAvailable) {
        if (m_cxyFixed.cy == 0) {
            // Jeffery: 当未指定高度时，使用默认字体的高度，并且对宽度进行DPI缩放
            //
            CDuiSize s;
            s.cy = m_pManager->GetDefaultFontInfo()->tm.tmHeight + 14;

            if (m_pManager) {
                s.cx = m_pManager->GetDPIObj()->Scale(m_cxyFixed.cx);
            } else {
                s.cx = m_cxyFixed.cx;
            }
            return s;
    ...
    RECT CListHeaderItemUI::GetSepThumbRect() const {
        int iScaledSepWidth = m_iSepWidth;

        if (m_bSepWidthAdaptDpiScale && m_pManager)
            iScaledSepWidth = m_pManager->GetDPIObj()->Scale(iScaledSepWidth);

        if(iScaledSepWidth >= 0 )
            return CDuiRect(m_rcItem.right - iScaledSepWidth, m_rcItem.top, m_rcItem.right, m_rcItem.bottom);
        else
            return CDuiRect(m_rcItem.left, m_rcItem.top, m_rcItem.left - iScaledSepWidth, m_rcItem.bottom);
    }
    ...
    void CListHeaderItemUI::PaintStatusImage(HDC hDC) {
        ...
        if( !m_sSepImage.IsEmpty() ) {
            ...
            m_sSepImageModify.Format(_T("dest='%d,%d,%d,%d' adaptdpiscale='%s'"),
                                     rcThumb.left, rcThumb.top, rcThumb.right, rcThumb.bottom,
                                     m_bSepWidthAdaptDpiScale ? "true" : "false"
                                    );
    ...
    void CListHeaderItemUI::PaintText(HDC hDC) {
        ...
        if( m_bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, sText, m_dwTextColor, NULL, NULL, nLinks, m_iFont, m_uTextStyle);
        else
    ...
    void CListElementUI::DrawItemBk(HDC hDC, const RECT &rcItem) {
        ...
        if ( pInfo->dwLineColor != 0 ) {
            if(pInfo->bShowRowLine) {
                RECT rcLine = { m_rcItem.left, m_rcItem.bottom - pInfo->dwRowLineHeight, m_rcItem.right, m_rcItem.bottom - pInfo->dwRowLineHeight };
                CRenderEngine::DrawLine(hDC, rcLine, pInfo->dwRowLineHeight, GetAdjustColor(pInfo->dwLineColor));
            }

            if(pInfo->bShowColumnLine) {
                for( int i = 0; i < pInfo->nColumns; i++ ) {
                    RECT rcLine = { pInfo->rcColumn[i].right - pInfo->dwColumnLineWidth,
                                    m_rcItem.top,
                                    pInfo->rcColumn[i].right - pInfo->dwColumnLineWidth,
                                    m_rcItem.bottom
                                  };
                    CRenderEngine::DrawLine(hDC, rcLine, pInfo->dwColumnLineWidth, GetAdjustColor(pInfo->dwLineColor));
                }
            }
        }
    }
    ...
    SIZE CListLabelElementUI::EstimateSize(SIZE szAvailable) {
        ...
        if (m_pManager) {
            ...
            if (cXY.cx == 0) {
                ...
                cXY.cx = m_pManager->GetDPIObj()->Scale(cXY.cx);
            }
        }
    ...
    void CListLabelElementUI::DrawItemText(HDC hDC, const RECT &rcItem) {
        ...
        if( pInfo->bShowHtml )
            CRenderEngine::DrawHtmlText(hDC, m_pManager, rcText, sText, iTextColor, NULL, NULL, nLinks, pInfo->nFont, pInfo->uTextStyle);
        else
    ...
    LPCTSTR CListTextElementUI::GetText(int iIndex) const {
        ...
        if( pText ) {
            ...
            return CResourceManager::GetInstance()->GetText(pText->GetData());
        }
        ...
        if( event.Type == UIEVENT_SETCURSOR ) {
            for( int i = 0; i < m_nLinks; i++ ) {
                if( ::PtInRect(&m_rcLinks[i], event.ptMouse) ) {
                    ::SetCursor(::LoadCursor(NULL, MAKEINTRESOURCE((DWORD)IDC_HAND)));
                    return;
                }
            }
        }
    ...
    void CListTextElementUI::DrawItemText(HDC hDC, const RECT &rcItem) {
        ...
        for( int i = 0; i < pInfo->nColumns; i++ ) {
            RECT rcItem2 = { pInfo->rcColumn[i].left, m_rcItem.top, pInfo->rcColumn[i].right, m_rcItem.bottom };
            rcItem2.left += pInfo->rcTextPadding.left;
            rcItem2.right -= pInfo->rcTextPadding.right;
            rcItem2.top += pInfo->rcTextPadding.top;
            rcItem2.bottom -= pInfo->rcTextPadding.bottom;
            ...
            if( pInfo->bShowHtml )
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rcItem2, strText, iTextColor, &m_rcLinks[m_nLinks], &m_sLinks[m_nLinks], nLinks, pInfo->nFont, pInfo->uTextStyle);
            else
                CRenderEngine::DrawText(hDC, m_pManager, rcItem2, strText, iTextColor, pInfo->nFont, pInfo->uTextStyle);
    ...
    void CListContainerElementUI::SetPos(RECT rc, bool bNeedInvalidate) {
        ...
        if (uListType == LT_TREE) {
            pList = (CListUI *)pList->CControlUI::GetInterface(DUI_CTR_LIST);
