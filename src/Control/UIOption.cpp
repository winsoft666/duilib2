...
namespace DuiLib {
    ...
    void COptionUI::PaintStatusImage(HDC hDC) {
        if(IsSelected()) {
            if(!m_sSelectedStateImage.IsEmpty() && m_nSelectedStateCount > 0) {
                const TDrawInfo *info = m_pManager->GetDrawInfo(m_sSelectedStateImage, _T(""));
                const TImageInfo *pImage = m_pManager->GetImageEx(info->sImageName, info->sResType, info->dwMask, info->bHSL);

                if (!pImage) {
                    pImage = m_pManager->GetImageEx(info->sImageName, info->sResType, info->dwMask, info->bHSL);
                }
                ...
                if(m_sSelectedImage.IsEmpty() && pImage != NULL) {
                    ...
                    if( szImage.cx > 0 && szImage.cy > 0 ) {
                        ...
                        if(m_nSelectedStateCount > 0) {
                            ...
                            m_sSelectedImage.Format(_T("res='%s' restype='%s' dest='%d,%d,%d,%d' source='%d,%d,%d,%d'"), 
                                info->sImageName, 
                                info->sResType, 
                                info->rcDest.left,
                                info->rcDest.top, 
                        ...
                        if(m_nSelectedStateCount > 1) {
                            ...
                            int iTop = rcSrc.top;
                            int iBottom = iTop + szStatus.cy;
                            m_sSelectedHotImage.Format(_T("res='%s' restype='%s' dest='%d,%d,%d,%d' source='%d,%d,%d,%d'"),
                                info->sImageName, 
                                info->sResType, 
                                info->rcDest.left,
                                info->rcDest.top,
                            ...
                            m_sSelectedPushedImage.Format(_T("res='%s' restype='%s' dest='%d,%d,%d,%d' source='%d,%d,%d,%d'"), 
                                info->sImageName, 
                                info->sResType, 
                                info->rcDest.left,
                                info->rcDest.top,
    ...
    void COptionUI::PaintText(HDC hDC) {
        if( (m_uButtonState & UISTATE_SELECTED) != 0 ) {
            ...
            if( m_bShowHtml )
                CRenderEngine::DrawHtmlText(hDC, m_pManager, rc, sText, IsEnabled() ? m_dwTextColor : m_dwDisabledTextColor, NULL, NULL, nLinks, iFont, m_uTextStyle);
            else
                CRenderEngine::DrawText(hDC, m_pManager, rc, sText, IsEnabled() ? m_dwTextColor : m_dwDisabledTextColor, iFont, m_uTextStyle);
