...
namespace DuiLib {
    ...
    void CTileLayoutUI::SetPos(RECT rc, bool bNeedInvalidate) {
        ...
        for( int it1 = 0; it1 < m_items.GetSize(); it1++ ) {
            ...
            if( (iCount % m_nColumns) == 0 ) {
                ...
                for( int it2 = it1; it2 < m_items.GetSize(); it2++ ) {
                    ...
                    if( iIndex == iCount || (iIndex + 1) % m_nColumns == 0 ) {
                        szAvailable.cx -= GetChildPadding() / 2;
                    } else {
                        szAvailable.cx -= GetChildPadding();
                    }
            ...
            rcTile.left += rcPadding.left + GetChildPadding() / 2;
            rcTile.right -= rcPadding.right + GetChildPadding() / 2;

            if( (iCount % m_nColumns) == 0 ) {
                rcTile.left -= GetChildPadding() / 2;
            }

            if( ( (iCount + 1) % m_nColumns) == 0 ) {
                rcTile.right += GetChildPadding() / 2;
            }
            ...
            if( (++iCount % m_nColumns) == 0 ) {
                ptTile.x = iPosX;
                ptTile.y += cyHeight + GetChildPadding();
                cyHeight = 0;
            } else {
                ptTile.x += cxWidth;
            }
